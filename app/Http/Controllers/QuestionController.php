...
class QuestionController extends Controller
{
   private $movies;
    ...
    public function create(){
         return Tmdb::getSearchApi()->searchMovies('ti');
   ...
   if($question->save()){
       ...
       foreach ($answers as $answer) {
           ...
           /*foreach ($options as $input) {
               //\DB::table('answers')->where(['id' => $answer->id])->update(['answer' => $input]);
               //$update = \App\Answer::where(['answer' => $answer->id])->update(['answer' => $input]);
                //$update->answer = $input;
                //$update->save();
           }*/
       }
...
public function randomize(Request $Requests){
    ...
    foreach ($unserialize as $value) {
       ...
       $trivia->fk_random = $randomize->id;
       $trivia->user_id = \Auth::user()->id;
       $trivia->save();
    }
    
   $start = \App\Trivia::where(['fk_random' => $randomize->id, 'user_id' => \Auth::user()->id])->first()->question_id;
   ...
   return view("questions.quiz", compact('question'))->with('fk_random', $randomize->id);
      
    
}

public function proceed($id, $answer, $fk_random){
    $trivia = \App\Trivia::where(['question_id' => $id, 'user_id' => \Auth::user()->id, 'fk_random' => $fk_random])->first();
    ...
    if($nextkey == count($unserialize)) {
        $score = (\App\Trivia::where(['user_id' => \Auth::user()->id, 'fk_random' => $fk_random, 'correct' => 1])->count()/50)*100;
        if($score < 30){
            $perfomance = "Fail";
...
public function revise($id){
    $trivia = \App\Trivia::where(['fk_random' => $id, 'user_id' => \Auth::user()->id])->first();
    $name = \App\Answer::find($trivia->answer)->answer;
    $question = Question::find($trivia->question_id);
    return view("questions.revise", compact('question', 'trivia', 'name'))->with('fk_random', $id);
...
public function nextrevise($id, $fk_random){
    ...
    $trivia = \App\Trivia::where(['fk_random' => $fk_random, 'question_id' => $unserialize[$nextkey], 'user_id' => \Auth::user()->id])->first();
   
    $name = \App\Answer::find($trivia->answer)->answer;
    $question = Question::find($unserialize[$nextkey]);
    
    return view("questions.revise", compact('question', 'trivia', 'name'))->with('fk_random', $fk_random);
}
...
function verification(){
    $check = \App\Verified::where(['user_id' => \Auth::user()->id, 'section_id' => \Auth::user()->section_id, 'subject_id'=> \Auth::user()->subject_id])->delete();
        ...
        if(\Auth::user()->role_id == 4){
            $questions = Question::where(['subject_id' =>\Auth::user()->subject_id, 'section_id' =>\Auth::user()->section_id])->get();
...
function prevrevise($id, $fk_random){
    $questions = \App\Random::find($fk_random)->random_questions;
    $unserialize = unserialize($questions);
    ...
    $nextkey = array_search($current, $unserialize) - 1;
    if($nextkey == -1) {
        return redirect("question/$fk_random/revise");
    }
...
function listsubject($id){
    ...
    return view('questions.subjects')->with($id);
}
...
function joingroup($id){
    ...
    $random->group_id =  $group->id;
    $random->random_questions = serialize($randoms);
    $random->save();
    
    $update = \App\Group::find($group->id);
    $update->random_id = $random->id;
    $update->save();
    
    $question = \App\Random::find($random->id);
    ...
    foreach ($unserialize as $value) {
       ...
       $trivia = new \App\Trivia();
       $trivia->question_id = $value;
       $trivia->fk_random = $random->id;
       $trivia->user_id = \Auth::user()->id;
       $trivia->save();
    }
    
   $start = \App\Trivia::where(['fk_random' => $random->id, 'user_id' => \Auth::user()->id])->first()->question_id;
   
   $question = Question::find($start);
   
    \Session::flash('success','The questions have been randomized, you are allowed to answer 50 questions, select the answer and the system will proceed to the next question');
   
   return view("questions.quiz", compact('question'))->with('fk_random', $random->id);
